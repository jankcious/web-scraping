...
wa.config.reportingStates = [{
  ...
  NAME: "preproduction",
  HOSTS: ["localhost", "qa.sephora.com", "preview.qa.sephora.com", "staging.merchantmail.net", "sandbox.qa.sephora.com", "origin.qa.sephora.com", "staging.illuminate.sephora.com", "staging.sephora.com", "dev.illuminate.sephora.com", "dev.sephora.com", "qa.illuminate.sephora.com", "m-qa.sephora.com", "m-qa.illuminate.sephora.com", "community.qa.sephora.com", "ebf.sephora.com", "qa-sephora.skedge.me", "dev.reserve.sephora.com", "dev-canada.sephora.com", "fr-dev-canada.sephora.com", "qa-canada.sephora.com", "fr-qa-canada.sephora.com", "fr-sephora-dev.onelink-translations.com", "fr-sephora-qa.onelink-translations.com", "gallery-qa.sephora.com", "sephora-semiprod.cashstar.com", "qa.colorcorrect.sephora.com", "sephora-color-correct-qa.herokuapp.com", "stage.colorcorrect.sephora.com", "sephora-color-correct-staging.herokuapp.com", "dev.colorcorrect.sephora.com", "sephora-color-correct-dev.herokuapp.com",  "qa.sephora.sg","qa.sephora.my","qa.sephora.co.id","qa.sephora.co.th","qa.sephora.com.au","qa.sephora.com.bn","qa.sephora.nz","qa.sephora.hk", new RegExp("\\d+\\.\\d+\\.\\d+\\.\\d+")],
        ...
        */ 
        ...
        if(pSignInStatus && pSignInStatus !== wa.signInStatus){
            ...
            if(wa.signInStatus == "signed in"){
                ...
                wa.setCookie("userJustSignedIn", wa.userJustSignedIn);
            }
        }
        
        if (window.sessionStorage) {
          var registrationObj = JSON.parse(sessionStorage.getItem('registered'));

          if (registrationObj.registerAccount) {
            wa.registerAccount = true;
          }
          wa.biOptIn = registrationObj.biOptIn === "BI" ? true : false;
          if (typeof registrationObj.emailOptIn !== "undefined") {
            wa.emailOptIn = true;
          }
          if (registrationObj.sawCaptcha) {
            wa.sawCaptcha = true;
          }
        ...
        if(!wa.ajaxOnlyLoadPages()){
          ...
          wa.setCookie("prevPageName", digitalData.page.sephoraPageInfo.pageName || wa.pageName);
        }
  } catch (e) {}
};
  ...
  try {
    ...
    if (wa.getQueryParam('om_mmc', wa.cURL)) { 
      ...
      if (wa.campaign.indexOf("ppc")!=-1){
        wa.setCookie("originPaidSearch", "true", 7, true);//Days may need to be adjusted
      }
    ...
    wa.emailHarmonyDeploymentId = wa.getQueryParam('emtc2', wa.cURL);

    //Harmony Link ID
    wa.emailHarmonyLinkId = wa.getQueryParam('emlid', wa.cURL);

    //Harmony Customer Key
    wa.emailHarmonyCustomerKey = wa.getQueryParam('emcid', wa.cURL);
    ...
    if (wa.getQueryParam('icid', wa.cURL)) { 
      wa.internalCampaign = wa.oldInternalCampaign = wa.removeSpecialChars( wa.getQueryParam('icid', wa.cURL) );
    } else if (wa.getQueryParam('icid2', wa.cURL)) {
      wa.internalCampaign = wa.removeSpecialChars( wa.getQueryParam('icid2', wa.cURL) );
      if (wa.internalCampaign.indexOf('_anaPath_') !== -1) { 
        var prevPath = digitalData.page.previousPageInfo.pageName || wa.prevPageName || "";
        wa.internalCampaign = wa.internalCampaign.replace(/_anaPath_/i, '_' + prevPath + '_');
      }
    } else if (wa.getQueryParam('int_cid', wa.cURL)) {
      wa.internalCampaign = wa.oldInternalCampaign = wa.removeSpecialChars( wa.getQueryParam('int_cid', wa.cURL) );
    }
    // google search rank
...
wa.world = function(){
    if ( $("[data-analytics-category]").length > 0 ){
        return $("[data-analytics-category]").first().data("analytics-category").toLowerCase();
    }
    else {return ""}
};

/* Aggregate detection from various pages so we can just add whatevever was found at once */
wa.existsOnPage = "";
